...
This directory contains a Keyboard file and a Speech to Text interface that uses the
...
In this application, we just put the Service Account in the client for ease of use. The app still gets
an access token using the service account credential, and use the token to call the API, so you can
see how to do so.

In order to try out this application, visit the [Cloud Console](https://console.cloud.google.com/), and
navigate to:
...
This application uses ProGuard to decrease the number of methods generated by the gRPC library. It is
